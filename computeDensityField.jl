...
function computeAverageSpeedField(indsArray, xlim, ylim, dxy, eFillVal)
    # Compute a field as given by xlim, ylim and dxy with average U and V values 
    # of individuals per cell, weighted by the individuals' N values:
    # weighted by their N value per grid cell:
    xrng = range(start=xlim[1], stop=xlim[2], step=dxy)
    yrng = range(start=ylim[1], stop=ylim[2], step=dxy)
    #field = zeros(Float64, length(xrng), length(yrng))
    fieldU = zeros(length(xrng), length(yrng))
    fieldV = zeros(length(xrng), length(yrng))
    fieldN = zeros(length(xrng), length(yrng))
    for i = 1:length(indsArray)
        ix = Int(floor((indsArray[i].x - xlim[1])/dxy))
        iy = Int(floor((indsArray[i].y - ylim[1])/dxy))
        ...
        if ix>=1 && iy>=1 && ix<=size(fieldU,1) && iy<=size(fieldU,2)
            # Add n to the density field:
            fieldN[ix, iy] = fieldN[ix, iy] + indsArray[i].n
            # Add n* to the summed speed fields:
            fieldU[ix, iy] = fieldU[ix, iy] + indsArray[i].n * indsArray[i].v_x
            fieldV[ix, iy] = fieldV[ix, iy] + indsArray[i].n * indsArray[i].v_y
        end
    end
    # Then divide the fummed energy field by the density field, leaving 0 in cells
    # with no individuals:
    fieldU = fieldU ./ fieldN
    fieldV = fieldV ./ fieldN
    for i=1:length(fieldU)
        if isnan(fieldU[i])
            fieldU[i] = eFillVal
        end
        if isnan(fieldV[i])
    ...
    return fieldU, fieldV, xrng, yrng
end
